...
void Liste<T>::vider()
{
    ...
    if (derniere){
        ...
        Cellule* c= derniere->suivante;
        while(c!=derniere){
            c=c->suivante;
            delete derniere->suivante;
            derniere->suivante =c;
        }
        delete derniere;
        derniere =nullptr;
...
typename Liste<T>::Iterateur Liste<T>::inserer(const T& e, const Iterateur& i)
{
    ...
    /*
      ...
      *** implementation naive de inserer(...)
      Cellule** c = &premiere;
      for (int i =0; i<position; i++){
       assert(*c!=nullptr);
       c=&((*c->suivante ));
      }
      *c = new Cellule(item, *c);



      // insertion apres 

      void Liste<T>::insererApres(const T& element, Cellule* position){
...
typename Liste<T>::Iterateur Liste<T>::inserer_debut(const T& e)
{
    // À compléter.
    return fin();
    // premiere = new Cellule(element, premiere);   // pour la representation naive de liste 
}
